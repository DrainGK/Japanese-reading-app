...
export function KanjiPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const mapped: KanjiItem[] = data.assignments
        .map((assignment) => {
          const subject = data.subjects.get(assignment.data.subject_id);
          if (!subject || subject.type !== 'kanji') return null;
        ...
        .sort((a, b) => a.level - b.level || a.char.localeCompare(b.char, 'ja'));
  ...
  return (
    <div className="container max-w-6xl mx-auto py-6 px-4 pb-24 md:pb-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Kanji</h1>
      <p className="text-gray-600 mb-6">Showing unlocked kanji only, grouped by WaniKani level.</p>

      {loading ? (
        <div className="card">Loading kanji...</div>
      ) : items.length === 0 ? (
        <div className="card">No unlocked kanji yet.</div>
      ) : (
        <>
          <div className="mb-4 max-w-xs">
            <label htmlFor="kanji-level" className="mb-2 block text-sm font-medium text-gray-700">
              Level
            </label>
            <select
              id="kanji-level"
              ...
              <tbody>
                {pageRows.map((row) => (
                  <tr key={row.id} className="border-b last:border-b-0">
                    <td className="px-3 py-2 text-2xl">{row.char}</td>
